...
export function remove(name, progress_cb) {
    const progress = new ProgressReporter(0, 1, progress_cb);
    return PK.find_file_packages([name], progress.progress_reporter).then(pkgnames => PK.remove_packages(pkgnames, progress.progress_reporter)).then(reload_bridge_packages);
}
